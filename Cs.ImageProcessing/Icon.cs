        ...
        private static bool ConvertToIcon(Stream input, Stream output, int size = 16, bool preserveAspectRatio = false)
        {
            ...
            if (inputBitmap != null)
            {
                ...
                if (preserveAspectRatio)
                {
                    int posX;
                    int posY;
                    Resize.ResizeAlgorithm(out newWidth, out newHeight, out posX, out posY, size, size, inputBitmap.Width, inputBitmap.Height);
                    //width = size;
                ...
                Bitmap newBitmap = new Bitmap(inputBitmap, new Size(newWidth, newHeight));
                if (newBitmap != null)
                {
                    ...
                    using (MemoryStream memoryStream = new MemoryStream())
                    {
                        ...
                        if (output != null && iconWriter != null)
                        {
                            ...
                            // image entry 1
                            // 0 image width
                            iconWriter.Write((byte)newWidth);
                            // 1 image height
                            iconWriter.Write((byte)newHeight);
